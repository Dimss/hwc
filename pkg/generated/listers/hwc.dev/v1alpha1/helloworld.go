...
type HelloWorldLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.HelloWorld, err error)
...
// List lists all HelloWorlds in the indexer.
func (s *helloWorldLister) List(selector labels.Selector) (ret []*v1alpha1.HelloWorld, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.HelloWorld))
	})
	return ret, err
}
...
type HelloWorldNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.HelloWorld, error)
...
// List lists all HelloWorlds in the indexer for a given namespace.
func (s helloWorldNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.HelloWorld, err error) {
...
// Get retrieves the HelloWorld from the indexer for a given namespace and name.
func (s helloWorldNamespaceLister) Get(name string) (*v1alpha1.HelloWorld, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("helloworld"), name)
	}
	return obj.(*v1alpha1.HelloWorld), nil
}
